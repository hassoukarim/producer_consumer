...
class Producer(Thread):
    ...
    """
    def __init__(self, buffer: List[np.array], condition: Condition, max_len: int, n_data: int):
        super().__init__()
        ...
        for i in range(self.n_data + 1):
            with self.condition:
                ...
                if i != self.n_data:
                    ...
                    self.buffer.append(None) # Add a dummy None token to signal that the consumer's stream of data finished
